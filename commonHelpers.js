import{r as i}from"./assets/support-03e6a105.js";import{a as l,i as f}from"./assets/vendor-e2ce0d38.js";class d{constructor(){this.BASE_URL="https://books-backend.p.goit.global/books"}async getCategoriesList(){const e=this.BASE_URL+"/category-list";return(await l.get(e)).data}async getPopularBooks(){const e=this.BASE_URL+"/top-books ";return(await l.get(e)).data}async getSelectedCategory(e){const o=this.BASE_URL+`/category?category=${e}`;return(await l.get(o)).data}async getBookDetailedInfo(e){const o=this.BASE_URL+`/${e}`;return(await l.get(o)).data}}function v(t){const e=document.querySelector(".modal-btn");let o=JSON.parse(localStorage.getItem("shoppingList"))||[],s;const n=o.findIndex(a=>t._id===a._id);n===-1?(o.push(t),s="remove from shopping list"):(o.splice(n,1),s="add to shopping list"),localStorage.setItem("shoppingList",JSON.stringify(o)),e.textContent=s}function y(t){const e=document.querySelector(".modal-btn");let o=JSON.parse(localStorage.getItem("shoppingList"))||[],s;o.findIndex(a=>t._id===a._id)===-1?s="add to shopping list":s="remove from shopping list",e.textContent=s}function L(t){return`<div class="modal-backdrop">
  ...
  </li>`}function S(t){return`<ul class='all-categories-main'>${t.map($).join("")}</ul>`}function x(t){const e=S(t);i.allCategoriesContainer.insertAdjacentHTML("beforeend",e)}function w(t){if(t.textContent!=="All categories"){const e=`<h2 class="title-book-section">${t.textContent}</h2>`;i.allCategoriesContainer.insertAdjacentHTML("beforeend",e),console.log(e),m(),console.log("ok")}}function m(){document.querySelectorAll(".title-book-section").forEach(function(e){const o=e.textContent.trim(),s=o.lastIndexOf(" "),n=o.slice(0,s)+'<span class="last-word">'+o.slice(s)+"</span>";e.innerHTML=n})}function g(){f.error({title:"",backgroundColor:"rgb(230, 92, 33)",message:"there are no books in this category",position:"topLeft"})}const A=new d;async function p(t){if(t.textContent==="All categories"||t==="All categories"){T("Best Sellers Books");let e;try{e=await A.getPopularBooks()}catch{console.log("error"),g()}const o=e.map(s=>`
      <h3 class="book-author popular-title">${s.list_name}</h3>
<ul class="all-categories-main">`+s.books.map(n=>{const{book_image:a,title:u,author:b,_id:h}=n;return`<li class="book-item" data-id="${h}">
       <div class="book-item-container">
           <div class="book-card-container"> 
            <img class="book-card" src="${a}" alt="${u}">
               <p class="book-card-hover-text">QUICK VIEW</p>
           </div>
           <div class="book-details">
