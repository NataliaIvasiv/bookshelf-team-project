import{r as n}from"./assets/support-d95aed48.js";import{a as r,i as y}from"./assets/vendor-e2ce0d38.js";class m{constructor(){this.BASE_URL="https://books-backend.p.goit.global/books"}async getCategoriesList(){const t=this.BASE_URL+"/category-list";return(await r.get(t)).data}async getPopularBooks(){const t=this.BASE_URL+"/top-books ";return(await r.get(t)).data}async getSelectedCategory(t){const o=this.BASE_URL+`/category?category=${t}`;return(await r.get(o)).data}async getBookDetailedInfo(t){const o=this.BASE_URL+`/${t}`;return(await r.get(o)).data}}function L(e){return`<div class="modal-backdrop">
    ...
    </div>`}function C(e){document.body.insertAdjacentHTML("beforeend",e)}function T(){const e=document.querySelector(".modal-backdrop"),t=document.querySelector(".modal-close-btn");document.addEventListener("keydown",o=>{o.key==="Escape"&&c()}),t.addEventListener("click",o=>{c()}),e.addEventListener("click",o=>{o.target===e&&c()})}function c(){const e=document.querySelector(".modal-backdrop"),t=document.querySelector(".modal-close-btn");e&&e.parentNode&&e.parentNode.removeChild(e),t&&(document.removeEventListener("keydown",o=>{console.log("listener is remove")}),t.removeEventListener("click",o=>{console.log("listener is remove")}),e.removeEventListener("click",o=>{console.log("listener is remove")})),document.body.style.overflow=null}function x(){const e=document.querySelector(".modal-success-text"),t=document.querySelector(".modal-btn");if(!e){if(t.textContent.trim().toLowerCase()==="remove from shopping list")return;w(`<p class="modal-success-text">Congratulations! You have added the book to the shopping list.
...
<ul class="all-categories-main">`+s.books.map(a=>{const{book_image:i,title:k,author:f,_id:v}=a;return`<li class="book-item" data-id="${v}">
  ...
  </li>`}).join("")+`</ul><button class='see-more-btn' data-id='${s.list_name}'>See more</button>`).join("");n.allCategoriesContainer.insertAdjacentHTML("beforeend",o)}}function j(e){const t=`<h2 class="title-book-section">${e}</h2>`;n.allCategoriesContainer.insertAdjacentHTML("afterbegin",t),p()}const H=document.querySelector(".categories-list-main"),U=new m;function N(e){return e.map(t=>`<li class='categories-list-item'><a href="">${t.list_name}</a></li>`).join("")}function P(e){const t=N(e);H.insertAdjacentHTML("beforeend",t)}async function W(){let e;try{e=await U.getCategoriesList()}catch(t){console.log(t);return}P(e)}const d=document.getElementById("scrollUpBtn");function z(){document.body.scrollTop>80||document.documentElement.scrollTop>80?d.style.display="block":d.style.display="none"}function D(){window.scrollTo({top:0,behavior:"smooth"})}window.addEventListener("scroll",z);d.addEventListener("click",D);const u=new m;let l;window.addEventListener("load",async()=>{_(),await W(),await h("All categories"),E()});n.categoriesListMain.addEventListener("click",J);async function J(e){e.preventDefault();let t;if(n.allCategoriesContainer.innerHTML="",e.target!==e.currentTarget){l=e.target.closest("li"),K(l),await h(l);try{t=await u.getSelectedCategory(l.textContent)}catch{console.log("error"),g()}I(l),b(t)}}function K(e){const t=document.querySelectorAll(".categories-list-item");for(const o of t)o.textContent===e.textContent?o.classList.add("categories-list-active"):o.classList.remove("categories-list-active")}n.allCategoriesContainer.addEventListener("click",async e=>{if(e.target.nodeName!="H2")if(e.target.nodeName=="BUTTON"){n.allCategoriesContainer.innerHTML="";const t=e.target.closest(".see-more-btn").dataset.id;let o;try{o=await u.getSelectedCategory(t),M(t),b(o)}catch{console.log("error"),g()}}else{const t=e.target.closest(".book-item").dataset.id,o=await u.getBookDetailedInfo(t),s=L(o);C(s),document.body.style.overflow="hidden",B(o),document.querySelector(".modal-btn").addEventListener("click",i=>{i.preventDefault(),x(),S(o)}),T()}});
