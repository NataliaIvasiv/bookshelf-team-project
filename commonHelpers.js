import{r as s}from"./assets/header-60fbfe01.js";import{a as r,i as g}from"./assets/vendor-fe89f55d.js";class u{constructor(){this.BASE_URL="https://books-backend.p.goit.global/books"}async getCategoriesList(){const e=this.BASE_URL+"/category-list";return(await r.get(e)).data}async getPopularBooks(){const e=this.BASE_URL+"/top-books ";return(await r.get(e)).data}async getSelectedCategory(e){const o=this.BASE_URL+`/category?category=${e}`;return(await r.get(o)).data}async getBookDetailedInfo(e){const o=this.BASE_URL+`/${e}`;return(await r.get(o)).data}}function k(t){const e=document.querySelector(".modal-btn");let o=JSON.parse(localStorage.getItem("shoppingList"))||[],n;const a=o.findIndex(i=>t._id===i._id);a===-1?(o.push(t),n="remove from shopping list"):(o.splice(a,1),n="add to shopping list"),localStorage.setItem("shoppingList",JSON.stringify(o)),e.textContent=n}function b(t){const e=document.querySelector(".modal-btn");let o=JSON.parse(localStorage.getItem("shoppingList"))||[],n;o.findIndex(i=>t._id===i._id)===-1?n="add to shopping list":n="remove from shopping list",e.textContent=n}function f(t){return`<div class="modal-backdrop">
  ...
  </li>`}function A(t){return t.map(T).join("")}function m(t){S();const e=A(t);s.categoriesMain.insertAdjacentHTML("beforeend",e)}function C(t){if(t.textContent!=="All categories"){const e=`${t.textContent}`;s.categoriesMainTitle.insertAdjacentHTML("beforeend",e)}}function S(){document.querySelectorAll(".title-book-section").forEach(function(e){const o=e.textContent.trim(),n=o.lastIndexOf(" "),a=o.slice(0,n)+'<span class="last-word">'+o.slice(n)+"</span>";e.innerHTML=a})}function x(){g.error({title:"",backgroundColor:"rgb(230, 92, 33)",message:"there are no books in this category",position:"topLeft"})}const M=new u;async function p(t){if(t.textContent==="All categories"||t==="All categories"){I("Best Sellers Books");try{(await M.getPopularBooks()).map(o=>{w(o)})}catch{console.log("error"),x()}}}function $(t){return`<li class="li-title"><h3 class="book-author popular-title">${t.list_name}</h3></li>`}function w(t){s.categoriesMain.insertAdjacentHTML("beforeend",$(t)),m(t.books)}function I(t){const e=`${t}`;s.categoriesMainTitle.insertAdjacentHTML("afterbegin",e)}const d=new u;async function _(){let t;try{t=await d.getCategoriesList()}catch(e){console.log(e);return}B(t)}_();let l;window.addEventListener("load",p("All categories"));s.categoriesListMain.addEventListener("click",q);async function q(t){t.preventDefault();let e;if(s.categoriesMain.innerHTML="",s.categoriesMainTitle.innerHTML="",t.target!==t.currentTarget){l=t.target.closest("li"),p(l);try{e=await d.getSelectedCategory(l.textContent),console.log(e)}catch{console.log("error"),emptyPage()}C(l),m(e)}}s.categoriesMain.addEventListener("click",async t=>{if(t.target===t.currentTarget)return;const e=t.target.closest(".book-item").dataset.id,o=await d.getBookDetailedInfo(e),n=f(o);h(n),b(o),document.querySelector(".modal-btn").addEventListener("click",i=>{i.preventDefault(),k(o)}),L()});
